...
public class ContextInitializer implements ContextBuilder<Object> {
	...
	int parcelNumber;
	int sortingCenterNumber;
	int parcelMachinesNumber;
	int agencyNumber;
	...
	public Context<Object> build(Context<Object> context) {
		...
		parcelNumber = (Integer)parm.getValue("parcelNumber");
		sortingCenterNumber = (Integer)parm.getValue("sortingCenterNumber");
		parcelMachinesNumber = (Integer)parm.getValue("parcelMachineNumber");
		agencyNumber = (Integer)parm.getValue("agencyNumber");
		...
		for(int i = 0; i < parcelNumber; i++) {
			...
			Coordinate coord = new Coordinate(19.90 + 0.05 * Math.random(), 50.03 + 0.05 * Math.random());
		...
		for(int i = 0; i < sortingCenterNumber; i++) {
			...
			Coordinate coord = new Coordinate(19.90 + 0.2 * Math.random(), 50.03 + 0.2 * Math.random());
			Point geom = fac.createPoint(coord);
			geography.move(agent, geom);
		...
		for(int i = 0; i < parcelMachinesNumber; i++) {
			...
			Coordinate coord = new Coordinate(19.90 + 0.01 * Math.random(), 50.03 + 0.01 * Math.random());
			Point geom = fac.createPoint(coord);
			geography.move(agent, geom);
			for(int j = 0; j < networkObjects.size(); j++) {
				net.addEdge(networkObjects.get(j), agent);
		...
		for(int i = 0; i < agencyNumber; i++) {
			...
			Coordinate coord = new Coordinate(19.90 + 0.03 * Math.random(), 50.03 + 0.03 * Math.random());
			Point geom = fac.createPoint(coord);
			geography.move(agent, geom);
			for(int j = 0; j < networkObjects.size(); j++) {
				net.addEdge(networkObjects.get(j), agent);
